...
// ==== Pindahkan type-guard ke module scope (bukan di dalam komponen) ====
...
export default function KioskScanPanel({
  ...
  // Hanya video constraints (sesuai tipe Scanner)
  const videoConstraints: MediaTrackConstraints = useMemo(
    () => ({ facingMode: { ideal: "environment" } }),
    []
  );

  // Fungsi ekstrak kode (stabil, tanpa dependency lokal)
  const extractCode = useCallback((value: unknown): string => {
  ...
  const handleDetected = useCallback(
    async (scanned: unknown) => {
      ...
      // Cooldown & cegah payload sama berulang
      if (now - cooldownRef.current < 1200 || code === lastCodeRef.current) return;
      ...
      try {
        if (code.startsWith("{")) {
          const obj = JSON.parse(code) as { uid?: unknown };
          if (typeof obj.uid === "string") userId = obj.uid;
        } else if (code.includes("uid=")) {
