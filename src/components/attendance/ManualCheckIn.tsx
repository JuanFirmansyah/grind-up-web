...
export default function ManualCheckIn({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok && res.duplicate)      { setStatus("dup"); setMsg("Sudah check-in hari ini"); }
    else if (res.ok)                  { setStatus("ok");  setMsg("Check-in berhasil"); }
    else if (res.reason === "membership_inactive") { setStatus("inactive"); setMsg("Membership tidak aktif"); }
    else                              { setStatus("err"); setMsg("Gagal check-in"); }
    setTimeout(() => { setStatus("idle"); setMsg(""); }, 1500);
  }
  ...
  return (
    <div className="max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2">
        ...
        <button type="button" className="flex items-center gap-2 px-4 py-2 rounded-xl border hover:bg-neutral-50" onClick={() => alert("Form Visitor/Drop-in: TODO")}>
      ...
      {status !== "idle" && (
        <div className={`mt-2 text-sm ${
          ...
          status === "dup" ? "text-amber-600" :
          status === "inactive" ? "text-red-600" : "text-neutral-600"
        }`}>
          {msg}
        </div>
      )}
      <p className="text-xs text-neutral-500 mt-3">Catatan: input di sini adalah <b>docId</b> pada koleksi <code>users</code>.</p>
    </div>
  );
}
